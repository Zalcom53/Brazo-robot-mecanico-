...
void brazoMain()
{
  ...
  int p = analogRead(Eje_L1); // Base
  if ( p < 200 )angulo1 = angulo1 - salto;
  else if (p > 800) angulo1 = angulo1 + salto;
  angulo1 = constrain(angulo1, 30, 180);
  servo1.write(angulo1);
  
  int s = analogRead(Eje_L2); //HOMBRO
  if ( s < 200 ) angulo2 = angulo2 + salto;
  ...
  angulo3 = constrain(angulo3, 75, 180); //75 180
  servo3.write(angulo3);
  
  int r = analogRead(Eje_R2); // Pinza
  if ( r < 200 ) angulo4 = angulo4 - salto;
  else if (r > 800) angulo4 = angulo4 + salto;
  angulo4 = constrain(angulo4, 145, 165); //135 175
